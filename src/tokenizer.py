...
class CustomTokenizer(BaseModel):
    """Tokenizer utilisant une approche gloutonne (Greedy Longest Match)."""
    ...
    def load_vocab(self) -> 'CustomTokenizer':
        """Charge le vocabulaire et prépare les structures de données
        post-instanciation."""
        try:
            ...
            # 2. Tri des tokens du plus long au plus court pour la méthode
            # encode()
            self.sorted_tokens = sorted(self.vocab.keys(),
                                        key=len,
                                        reverse=True)

            logger.info(f"Tokenizer custom chargé avec {len(self.vocab)} "
                        "tokens.")
